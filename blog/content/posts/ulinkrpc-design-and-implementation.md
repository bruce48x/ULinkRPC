---
title: ULinkRPC 设计思路与底层实现拆解：为什么它能把游戏客户端和 .NET 双向 RPC 做得既强类型又不笨重
...
这篇文章不再讲环境搭建，只讲设计本身：它为什么这样分层，为什么把 C# 契约作为通信边界，双向通信到底是怎么落到一条连接上的。

## 先给一句总纲：ULinkRPC 其实是“共享 C# 契约 + 胶水代码生成 + 帧级运行时”的组合

先记住一句话就够了：**服务端和客户端共享同一份 C# 契约，生成器把契约翻译成胶水代码，运行时再把胶水代码变成真实网络收发。**
...
这对短期 demo 没什么，但对多人协作、长期迭代、客户端和服务端版本错位这些真实场景并不友好。
...
所以 ULinkRPC 不是为了“代码生成”而代码生成。生成器只是支撑共享 C# 契约落地的手段，它很务实地换取三样东西：
...
那 ULinkRPC 就能直接把同一份 C# 类型系统延伸到服务端和客户端通信边界上。
...
### 3. 你的服务端和客户端不是同一套 C# 技术栈

ULinkRPC 的出发点就是让 C# 服务端和 C# 游戏客户端共享同一份通信契约。如果你的客户端主要是 TypeScript、C++、Java、Go 或原生移动端，通常更适合 schema-first 或多语言 IDL 方案。
...
1. **用接口定义通信边界**
2. **用 source generator 填平 binder / proxy / facade 这类样板代码**
...
如果你正好在做 Unity + .NET 项目，这套思路会比较顺：写 C# 契约、正常编译、接 runtime，业务层只管实现接口和处理 callback。
