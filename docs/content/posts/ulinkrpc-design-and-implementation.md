...
但如果你准备把 ULinkRPC 真正放进项目里，通常还会继续追问：
...
这篇文章就不再教你搭环境，而是专门把 **ULinkRPC 的设计动机、分层方式、关键数据流、生成代码的作用、运行时的底层机制** 一层一层拆开。

目标只有一个：**让你在用它时，不只是“会接”，而是知道它为什么这么设计。**

## 先给一句总纲：ULinkRPC 其实是“契约驱动 + 代码生成 + 帧级运行时”的组合

如果只看表面，ULinkRPC 像一个 RPC 框架；但从设计上看，它其实是三层东西叠在一起：
...
也就是说，**生成层的职责，是把“接口定义”变成“可发包、可收包、可分发”的代码。**
...
所以你可以把 ULinkRPC 记成一句话：

> **Contract 决定语义，CodeGen 决定胶水，Runtime 决定传输。**

这也是它最核心的设计思想：**把“接口语义”和“网络实现”硬切开，中间靠自动生成代码桥接。**
...
真正要上线跑网络，肯定还是得落回字节流。所以 ULinkRPC 在 Contract 和 Transport 之间又加了一层非常关键的东西：**Envelope**。
...
### 为什么是这 5 种，而不是更复杂的一坨统一结构

因为 ULinkRPC 把“双向 RPC”拆成了两类完全不同的语义：

1. **请求-响应**：必须携带 `requestId`
2. **服务端推送**：不需要 `requestId`

于是：

- `Request` 里有 `RequestId + ServiceId + MethodId + Payload`
- `Response` 里有 `RequestId + Status + Payload + ErrorMessage`
- `Push` 里只有 `ServiceId + MethodId + Payload`
...
这种设计比“所有消息都塞到一个万能结构里”更直接，也更利于运行时做快速判断。
...
这就是 ULinkRPC 能自由切换 JSON / MemoryPack 的根本原因。

**协议头是框架统一标准；payload 编码可以替换。**
...
所以它不是那种“大而全”的网络框架，而是一套相当克制的设计：

- 上层给你强类型服务心智
- 中层给你自动生成胶水
- 下层给你统一帧协议和可替换基础设施

如果你正好在做 Unity + .NET 的项目，这套思路会非常顺：
...
这也是为什么我会把 ULinkRPC 理解成一句话：

> **它不是在帮你“偷懒发包”，而是在帮你把双向 RPC 的边界设计得更稳、更清晰、更能长期维护。**
