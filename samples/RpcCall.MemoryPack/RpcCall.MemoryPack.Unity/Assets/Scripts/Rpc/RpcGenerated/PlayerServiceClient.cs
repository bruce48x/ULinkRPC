...
namespace Rpc.Generated
{
    public sealed class PlayerServiceClient : IPlayerService
    {
        ...
        private static readonly RpcMethod<(string, int, bool), string> composeGreetingAsyncRpcMethod = new(ServiceId, 3);
        ...
        {
            return ComposeGreetingAsync(name, level, vip, CancellationToken.None);
        }

        public ValueTask<string> ComposeGreetingAsync(string name, int level, bool vip, CancellationToken ct)
        {
            return _client.CallAsync(composeGreetingAsyncRpcMethod, (name, level, vip), ct);
        }
