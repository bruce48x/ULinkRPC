...
public static class WsServerHostBuilderExtensions
{
    public static RpcServerHostBuilder UseWebSocket(
        ...
        string path = "/ws")
    {
        ...
        return builder.UseAcceptor(ct => WsConnectionAcceptor.CreateAsync(builder.ResolvePort(defaultPort), path, ct));
    }

    private sealed class WsConnectionAcceptor : IRpcConnectionAcceptor
    {
        ...
        public static async ValueTask<IRpcConnectionAcceptor> CreateAsync(int port, string path, CancellationToken ct)
        {
